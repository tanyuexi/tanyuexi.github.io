...
+ <mark>Feed-forward Neural Networks</mark> ([Fig2.1-1A](#Fig2.1-1))
   - 实际应用中最常见的类型
...
+ <mark>Recurrent Neural Networks</mark> ([Fig2.1-1B](#Fig2.1-1))
   - 被认为是适用于序列数据的自然模型
   - 神经元之间的连接是有向循环。神经元的输出可以回到自身。相当于很深的 Feed-forward Neural Networks，只不过每两层神经元之间的连接强度总是一样的
   - 这种结构使其具有复杂的动态变化，能够长时间“记住”信息，但是难以训练参数
   - 更接近生命体机制
+ <mark>Symmetrically Connected Networks</mark>
   - 与Recurrent Neural Networks相似，只不过神经元连接是对称的。即，连接是双向的，且连接强度相等
   - Hopfield 等发现，对称的网络比 Recurrent Networks 分析起来更容易
   - 但是由于它们遵循一个能量方程，所以它们能做的事情很有限
   - 没有隐藏神经元的对称网络被称为“Hopfield Nets”
   - 具有隐藏神经元的对称网络被称为“Boltzmann Machines”
      + 它比 Hopfield Nets 更加强大
      + 但是弱于Recurrent Neural Networks
      + 有一个优美的学习算法
...
+ 对同一特征运用多个检测单元。这叫“<mark>卷积神经网络（Convolutional Neural Nets）</mark>”，是现在神经网络应用的主流方法。详见下文

### 5.3 卷积神经网络

对一个可能发生了平移的图案，我们可以将其特征检测单元复制好几个，置于不同位置上，使得无论图案平移到哪里，至少有一个单元能够检测出来（[Fig5.3-1](#Fig5.3-1)）。当然，除了“位置”，我们也可以在“大小”和“方向”的层面复制检测单元，但这很难，而且代价很高。另外，我们还可以对同一个图案同时检测多个特征。总之，<mark>从本质上看，卷积神经网络只不过比基本神经网络在寻找目标权重的过程中多了一些限制条件</mark>。

<img src="{{ "/images/20160318-CNN-200x280.png" | prepend: site.baseurl }}" id="Fig5.3-1">

*Fig5.3-1 Convolutional Neural Nets.*

事实上，只要将 Backpropagation Algorithm 稍加修改，便能实现卷积神经网络：按照原方法计算每个权重的调整量后，令处于不同位置上的检测单元的实际调整量相等，即，对应权重的调整量之和。这样一来，只要某一处的检测单元能够满足条件，则所有位置上的检测单元都应是满足条件的（[Fig5.3-2](#Fig5.3-2)）。

<img src="{{ "/images/20160318-CNNweights-300x292.png" | prepend: site.baseurl }}" id="Fig5.3-2">

*Fig5.3-2 Constraints in Convolutional Neural Nets.*
...
*Fig5.3-3 LeNet using Convolutional Neural Nets.*
...
在 ILSVRC-2012 图像分类竞赛中，多伦多大学的 Alex Krizhevsky 在 LeNet 的启发下，通过利用深层卷积神经网络、GPU 和许多其他技巧，将错误率降低至 16.4%，超越第二名多达十个百分点。
